<template>
    <div class="nav-right tradeCenter">
        <section class="trade-group merchant-top">
            ...
            <a href="/#/otc/ad/create">{{$t('otc.publishAd')}}&gt;&gt;</a> -->

            <!-- 判断是否是认证商家 -->
            <!-- <a v-if='memberLevel' href='/#/otc/ad/create'>您已是认证商家，去发布广告吧</a> -->
            <a href="/#/identbusiness">{{$t('otc.identbusiness')}}</a>
        </section>
        <section class="list-content">
            <Tabs :value="tabPage"  @on-click="showItem">
                    ...
                    <div class="table-responsive list-table">
                        ...
                        <div style="margin: 10px;overflow: hidden">
                            <div style="float: right;">
                                <Page v-if="advertiment.ask.totalElement > 0" :total="advertiment.ask.totalElement" :current="1" @on-change="changePage" :page-size="advertiment.ask.pageNumber"></Page>
                            </div>
                        </div>
                    </div>
                </TabPane>
                <TabPane :label="$t('common.isell')" name="sell">
                   <div class="table-responsive list-table">
                        ...
                        <div style="margin: 10px;overflow: hidden">
                            <div style="float: right;">
                                <Page v-if="advertiment.bid.totalElement > 0" :total="advertiment.bid.totalElement" :current="1" @on-change="changePage" :page-size="advertiment.bid.pageNumber"></Page>
                            </div>
                        </div>
                    </div>
                </TabPane>
...
export default {
                            ...
                            return h('p', [
                                h('a', {
                                    ...
                                    on: {
                                        ...
                                        h('Button', {
                                            props: {
                                                type:  params.row.advertiseType == 0 ? 'success':'error',
                                                long: true,
                                            },
                                            style: {
                                                marginRight: '8px',
    ...
    methods: {
        ...
        updateLangData(){
                this.advertiment.columns[0].title = this.$t('common.memberName');
                this.advertiment.columns[1].title = this.$t('otc.transactions');
                this.advertiment.columns[2].title = this.$t('otc.payMode');
                this.advertiment.columns[3].title = this.$t('common.amount');
                this.advertiment.columns[4].title = this.$t('otc.price');
                this.advertiment.columns[5].title = this.$t('common.operate');
        },
        ...
        changePage(index){
            if (this.whichItem == 0) {
               this.loadAd(index,0,this.advertiment.bid);
            } else if (this.whichItem == 1) {
                this.loadAd(index,1,this.advertiment.ask);
            }
        },
        reloadAd(){
        ...
        // 认证商家
        isBusiness(){
            this.$http.post(this.host + this.api.uc.state).then(response => {
                const res=response.body;
                if(res.code==0){
                    const memberLevelNum=response.data.data.certifiedBusinessStatus;
                    if(memberLevelNum==2){
